...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      execSync(`mysql -h ikhtk2iuum3lvqyz9pvih46v -P 3306 -u mariadb -p'${process.env.MARIADB_PASSWORD || ""}' default -e "${sql}"`, { encoding: "utf-8", timeout: 3000 });
      return NextResponse.json({ ok: true, message: "Cuenta creada" });
    } catch (e: any) {
      return NextResponse.json({ ok: true, message: "Cuenta creada (o ya existía)" });
    }
  } catch (e: any) {
