function CategoryTag({ className = "", children }: { className?: string; children: React.ReactNode }) {
  return (
    <span className={`pixel pointer-events-none absolute left-3 top-3 z-10 rounded-sm border-2 px-2.5 py-1 text-[9px] tracking-widest ${className}`} aria-hidden="true">
      {children}
    </span>
  );
}
...
export function AnimeLayout({ items }: { items: CatalogItem[] }) {
...
export function GamingLayout({ items }: { items: CatalogItem[] }) {
...
export function CineLayout({ items }: { items: CatalogItem[] }) {
...
export function AccesoriosLayout({ items }: { items: CatalogItem[] }) {
  return (
...
export function MundialLayout({ items }: { items: CatalogItem[] }) {
  return (
    <div className="category-products">
...
export function DefaultLayout({ items }: { items: CatalogItem[] }) {
  return (
...
export const layouts: Partial<Record<CategoryId, (props: { items: CatalogItem[] }) => React.ReactNode>> = {
  anime: AnimeLayout,
  gaming: GamingLayout,
