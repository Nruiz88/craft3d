...
- **13/08/2026 — Mystery box · Contenido por pieza con cantidad**: la caja es un **bundle** con cantidades. Tag `box-include:<slug>[:<qty>]` (qty ≥ 1). `lib/mystery-box.ts`: `MysteryBoxInclude`/`MysteryBoxItem`, `getMysteryBoxIncludes` (parsea qty), `mysteryBoxIncludeTags`, `getMysteryBoxItems` (producto+qty, con fallback legacy pool:/box-exclude:) y `getMysteryPoolProducts` expande por qty. `MysteryPoolPreview`: `pieces` con `qty`, `total` = suma de unidades, `totalValue` = Σ precio×qty; se eliminan sorteo (`drawMysteryPiece`), `getMysteryRarityOdds` y las odds del UI. **Revelado**: `confirmMysteryRevealAction` ya no sortea — entrega TODAS las piezas de la caja, valida stock ≥ qty por pieza (error lista faltantes), descuenta qty de cada una y appenda una línea "🎁 Incluye: X" (quantity qty) por pieza. `drawMysteryPieceAction` y `RevealPieceResult` eliminados. `reveal-panel.tsx` reescrito: chips del contenido ×qty + botón único "🎁 Revelar caja". Email `sendMysteryRevealedEmail(order, pieces[], …)` lista todo el contenido. `product-form.tsx`: stepper −/+/0 por producto (fila resaltada si qty>0) + panel "N piezas · Valor de la caja: $X" (Σ precio×qty). `MysteryRevealItem`/Mis pedidos muestran ×qty. Copy de `/mysterybox` y vista de producto actualizadas ("Contenido de la caja"). Cero SQL.

- **13/08/2026 — Mystery box · Selección manual de piezas + valor de caja**: reemplaza el modelo "pool por categoría" por elección explícita. `lib/mystery-box.ts`: tag `box-include:<slug>` (con fallback legacy a `pool:`/`box-exclude:` para cajas existentes), `getMysteryBoxIncludes`/`mysteryBoxIncludeTags`/`mysteryBoxPoolLabel` ("N piezas"); se eliminan `pool:` y `box-exclude:` como modelo nuevo. `MysteryPoolPreview` gana `totalValue` (suma de precios del pool). `parseProductForm` limpia `pool:`/`box-exclude:`/`box-include:` y regraba `boxIncludes`. `product-form.tsx`: sección "Piezas de la caja" con buscador + filtro por categoría + checkbox por producto (emoji/precio/rareza) + panel "N piezas · Valor de la caja: $X" en vivo. **Stock**: se descuenta 1 unidad de la pieza al revelar (`decrementProductStock` ya en `confirmMysteryRevealAction`). Tienda: card y vista de producto muestran "🎁 Valor total de la caja: $X"; textos de `/mysterybox` y tabla/revelaciones usan el nuevo label. Cero SQL.
